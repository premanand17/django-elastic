...
class TastypieTest(ResourceTestCase):
    ...
    def setUp(self):
        ...
        # Create a cv
        self.cv = Cv.objects.create(cv_id=12, name='local_test',
                                    definition='local definition - test')
        self.db = Db.objects.create(db_id=1, name='test')
        self.dbxref = Dbxref.objects.create(dbxref_id=1, db_id=1,
                                            accession='XXX')
        self.cvterm = Cvterm.objects.create(cvterm_id=12, dbxref_id=1,
                                            cv_id=12, name='test cvterm',
                                            is_obsolete=0,
