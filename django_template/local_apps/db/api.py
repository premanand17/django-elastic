...
from db.models import Cvterm, Cv, Cvtermprop, Feature, Featureloc, Featureprop, Organism
#import logging
...
class CvtermFullResource(ModelResource):
    ...
    cvtermprops = fields.ToManyField('db.api.CvtermpropResource', 'cvtermprop_cvterm', full=True, null=True)

    class Meta:
        queryset = Cvterm.objects.select_related("cv")
        excludes = [ 'is_relationshiptype' ]
        filtering = { 
                     "cv": ALL_WITH_RELATIONS, 
                     "is_obsolete": ALL, 
                     "name": ALL }
        cache = SimpleCache(timeout=100)
...
class CvtermpropResource(ModelResource):
    cvterm = fields.ForeignKey(CvtermResource, 'cvterm', related_name='cvtermprop_cvterm')
    type_id = fields.ForeignKey(CvtermResource, 'type', related_name='cvtermprop_type')
...
class FeatureFullResource(ModelResource):
    featurelocs = fields.ToManyField('db.api.FeaturelocResource', 'featureloc_feature', related_name='featureloc_feature', full=True)
    featureprop = fields.ToManyField('db.api.FeaturepropResource', 'featureprop_set', full=True)
    type = fields.ForeignKey(CvtermResource, 'type', full=True)
    organism = fields.ForeignKey(OrganismResource, 'organism', full=True)

    class Meta:
        queryset = Feature.objects.all()
        excludes = [ 'residues', 'md5checksum', 'timeaccessioned', 'timelastmodified', 'is_analysis', 'is_obsolete' ]
        filtering = {
              'uniquename' : ALL,
...
class FeaturelocResource(ModelResource):
    ...
    feature = fields.ForeignKey(FeatureResource, 'feature', related_name='featureloc_feature', null=True)
    srcfeature = fields.ForeignKey(FeatureResource, 'srcfeature', related_name='featureloc_srcfeature', null=True)
    class Meta:
...
class FeaturelocFullResource(ModelResource):
    ''' Resource of all featurelocs '''
    feature = fields.ForeignKey(FeatureResource, 'feature', full = True)
    srcfeature = fields.ForeignKey(FeatureResource, 'srcfeature', null=True)
    class Meta:
        queryset = Featureloc.objects.select_related("feature", "srcfeature")
        excludes = [ 'is_fmax_partial', 'is_fmin_partial', 'locgroup', 'residue_info', 'phase' ]
        filtering = {
            'feature' : ALL_WITH_RELATIONS,
            'srcfeature' : ALL_WITH_RELATIONS,
        }
